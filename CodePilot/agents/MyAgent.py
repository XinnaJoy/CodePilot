...
from memory import WorkingMemory, SessionDB
...
def run_bash(command: str) -> str:
    ...
    try:
        # On Windows, try system encoding first (GBK), fallback to UTF-8
        if sys.platform == 'win32':
            r = subprocess.run(command, shell=True, cwd=WORKDIR,
                               capture_output=True, timeout=120)
            # Try to decode with system encoding (GBK on Chinese Windows)
            try:
...
TOOL_HANDLERS = {
    ...
    "mem_set": lambda **kw: WORKING_MEM.set(kw["key"], kw["value"]),
    "mem_get": lambda **kw: WORKING_MEM.get(kw["key"]),
    "save_session": lambda **kw: f"Snapshot ID: {SESSION_DB.save_snapshot(kw['session_id'], kw['messages'], WORKING_MEM.to_dict())}",
    "list_sessions": lambda **kw: json.dumps(SESSION_DB.list_all_snapshots(kw.get('limit', 10)) if not kw.get('session_id') else SESSION_DB.list_snapshots(kw['session_id'], kw.get('limit', 10)), indent=2),
    "get_session": lambda **kw: json.dumps(SESSION_DB.get_latest_snapshot(kw['session_id']), indent=2, default=str),
}
...
TOOLS = [
    ...
    {"name": "claim_task", "description": "Claim a task from the board.",
     "input_schema": {"type": "object", "properties": {"task_id": {"type": "integer"}}, "required": ["task_id"]}},
    {"name": "mem_set", "description": "Set working memory variable", "input_schema": {"type": "object", "properties": {"key": {"type": "string"}, "value": {"type": "string"}}, "required": ["key", "value"]}},
    {"name": "mem_get", "description": "Get working memory variable", "input_schema": {"type": "object", "properties": {"key": {"type": "string"}}, "required": ["key"]}},
    {"name": "save_session", "description": "Save conversation snapshot for recovery", "input_schema": {"type": "object", "properties": {"session_id": {"type": "string"}, "messages": {"type": "array"}}, "required": ["session_id", "messages"]}},
    {"name": "list_sessions", "description": "List saved session snapshots", "input_schema": {"type": "object", "properties": {"session_id": {"type": "string"}, "limit": {"type": "integer"}}}},
    {"name": "get_session", "description": "Get latest session snapshot", "input_schema": {"type": "object", "properties": {"session_id": {"type": "string"}}, "required": ["session_id"]}},
]
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if query.strip() == "/memory":
            ...
            print(json.dumps(SESSION_DB.get_stats(), indent=2))
            print("\n=== Working Memory ===")
            print(WORKING_MEM.render())
            continue
